...
def train(training_data_loader, indoor_test_loader,optimizer, epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        label_1 = F.interpolate(label, scale_factor = 0.5)
        mse_loss = criterion_mse(output2, label)
        loss = criterion_mse(output1, label_1) + mse_loss
        #loss = criterion_mse(output1, label_1) + mse_loss + 0.04*loss_networke(output2, label)+0.04*loss_networke(output1, label_1)
